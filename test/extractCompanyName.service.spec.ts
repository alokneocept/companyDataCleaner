...
describe('ExtractCompanyNameService', () => {
  ...
  beforeEach(() => {
    ...
    extractCompanyNameService = new ExtractCompanyNameService(emailValidatorService);
  });

  describe('findCompanyNames', () => {
    it('throw an error if data is not an array', async () => {
      await expect(extractCompanyNameService.findCompanyNames(null)).rejects.toThrowError('data must be an array.');
    });

    it('should find and set company name if email is valid', async () => {
      const rows = [
        { 'Email': 'test@neocepts.com', 'Company name': 'neocepts' }
      ];
    ...
    it('should log a warning if email is invalid', async () => {
      ...
      expect(consoleWarnSpy).toHaveBeenCalledWith('Invalid email: invalidemail');
    ...
    it('should show warning if phone number and email are the same', async () => {
      ...
      expect(consoleWarnSpy).toHaveBeenCalledWith('Phone number and email are the same: 9874897539');
      expect(consoleWarnSpy).toHaveBeenCalledWith('Phone number and email are the same: 8678957769');
  ...
  describe('getCompanyNameFromEmail', () => {
    it('should extract company name from email', () => {
      ...
      const companyName = extractCompanyNameService.getCompanyNameFromEmail(email);
